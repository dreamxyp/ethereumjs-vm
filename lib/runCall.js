...
const BN = ircUtil.BN
const fees = require('icjs-common')
...
module.exports = function (opts, cb) {
  ...
  function loadToAccount (done) {
    ...
    if (!toAddress) {
      ...
      createdAddress = toAddress = ircUtil.generateAddress(caller, newNonce.toArray())
  ...
  function subTxValue (cb) {
    ...
    stateManager.putAccountBalance(ircUtil.toBuffer(caller), newBalance, cb)
  }
  ...
  function addTxValue (cb) {
    ...
    stateManager.putAccount(ircUtil.toBuffer(toAddress), toAccount, cb)
  }
