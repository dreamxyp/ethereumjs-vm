...
function createHookedVm (opts, hooks) {
  ...
  function createAccountStorageTrie (address, cb) {
    var addressHex = hucUtil.addHexPrefix(address.toString('hex'))
    var storageTrie = new FallbackStorageTrie({
      fetchStorage: function (key, cb) {
        hooks.fetchAccountStorage(addressHex, hucUtil.addHexPrefix(key), cb)
      }
    })
    cb(null, storageTrie)
  }
...
function fromWebuProvider (provider, blockNumber, opts) {
  return createHookedVm(opts, {
    fetchAccountBalance: createRpcFunction(provider, 'huc_getBalance', blockNumber),
    fetchAccountNonce: createRpcFunction(provider, 'huc_getTransactionCount', blockNumber),
    fetchAccountCode: createRpcFunction(provider, 'huc_getCode', blockNumber),
    fetchAccountStorage: createRpcFunction(provider, 'huc_getStorageAt', blockNumber)
  })
...
FallbackStorageTrie.prototype.get = function (key, cb) {
  ...
  _super(key, function (err, value) {
    ...
    self._fetchStorage(keyHex, function (err, rawValue) {
      if (err) return cb(err)
      var value = hucUtil.toBuffer(rawValue)
      value = hucUtil.unpad(value)
      var encodedValue = hucUtil.rlp.encode(value)
