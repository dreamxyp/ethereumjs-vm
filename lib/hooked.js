...
const FakeMerklePatriciaTree = require('fake-icjs-mpt')
const VM = require('./index.js')
const ZERO_BUFFER = Buffer.alloc(32, 0)

module.exports = createHookedVm
module.exports.fromWebuProvider = fromWebuProvider
...
function createHookedVm (opts, hooks) {
  ...
  function createAccountStorageTrie (address, cb) {
    var addressHex = ircUtil.addHexPrefix(address.toString('hex'))
    var storageTrie = new FallbackStorageTrie({
      fetchStorage: function (key, cb) {
        hooks.fetchAccountStorage(addressHex, ircUtil.addHexPrefix(key), cb)
      }
    })
    cb(null, storageTrie)
  }
  ...
  function loadAccount (address, cb) {
    ...
    async.parallel({
      ...
      // not used but needs to be anything but the default (ircUtil.SHA3_NULL)
...
function fromWebuProvider (provider, blockNumber, opts) {
  return createHookedVm(opts, {
    fetchAccountBalance: createRpcFunction(provider, 'irc_getBalance', blockNumber),
    fetchAccountNonce: createRpcFunction(provider, 'irc_getTransactionCount', blockNumber),
    fetchAccountCode: createRpcFunction(provider, 'irc_getCode', blockNumber),
    fetchAccountStorage: createRpcFunction(provider, 'irc_getStorageAt', blockNumber)
  })
...
FallbackStorageTrie.prototype.get = function (key, cb) {
  ...
  _super(key, function (err, value) {
    ...
    self._fetchStorage(keyHex, function (err, rawValue) {
      if (err) return cb(err)
      var value = ircUtil.toBuffer(rawValue)
      value = ircUtil.unpad(value)
      var encodedValue = ircUtil.rlp.encode(value)
